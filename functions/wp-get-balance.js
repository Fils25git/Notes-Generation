...
export async function handler(event) {
  try {
    ...
    // 2️⃣ Total approved lessons purchased (from weekly_plan_payments)
    const paymentRes = await pool.query(
      'SELECT COALESCE(SUM(lessons),0) AS total_purchased FROM weekly_plan_payments WHERE user_id=$1 AND status=$2',
      [userId, 'approved']
    );
    const totalPurchased = parseInt(paymentRes.rows[0].total_purchased, 10);

    // 3️⃣ Total lessons used (lesson_usage table stays the same)
