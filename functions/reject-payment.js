...
export async function handler(event) {
  ...
  try {
    ...
    const resPayments = await client.query(
      "DELETE FROM payments WHERE id = ANY($1::int[]) AND status='pending'",
      [ids]
    );

    const resWeekly = await client.query(
      "DELETE FROM weekly_plan_payments WHERE id = ANY($1::int[]) AND status='pending'",
      [ids]
    );
    ...
    return { statusCode: 200, body: JSON.stringify({ success: true, message: "Pending payments cleared successfully." }) };
